...
#' }
#' @export
bwaIndex <- function(group=c("sudo","docker"), genome.folder=getwd(), genome.url=NULL, gtf.url=NULL, dbsnp.file=NULL, g1000.file=NULL, mode=c("General","GATK","miRNA","ncRNA"), mb.url.hairpin, mb.url.mature, mb.species=NULL, rc.version=NULL, rc.species=NULL, length=NULL){
...
### BWA index case for miRNA
	if(mode=="miRNA"){
	  ...
	  if(is.null(mb.species)){
	    ...
	    params <- paste("--cidfile ", genome.folder, "/dockerID -v ", genome.folder,":/data/scratch", " -d docker.io/repbioinfo/bwaindex bash /bin/bwa.index.sh ", genome.folder, " ", mode, " ", mb.url.hairpin, " ", mb.url.mature, " ", mb.species, sep="")
	  }
	}
