...
oncosnp <- function(group=c("sudo","docker"), data.folder, scratch.folder, sample.name=NULL, blood.name=NULL){
  ...
  if(is.null(blood.name)){
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",data.folder,":/data/ -v ", scratch.folder,":/scratch"," -d docker.io/repbioinfo/r340.2017.01 bash /bin/oncosnp_command.sh ", sample.name, sep="")
  }else{
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",data.folder,":/data/ -v ", scratch.folder,":/scratch"," -d docker.io/repbioinfo/r340.2017.01 bash /bin/oncosnp_command2.sh ", sample.name, " ", blood.name, sep="")
  }
