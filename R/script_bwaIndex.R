...
#'\dontrun{
#'     #running bwa index
#'     bwaIndex(group="sudo",genome.folder=getwd(),
#'     ensembl.url="ftp://ftp.ensembl.org/pub/release-87/fasta/homo_sapiens/dna/Homo_sapiens.GRCh38.dna.toplevel.fa.gz",
#'     threads=24)
#' }

bwaIndex <- function(group=c("sudo","docker"),genome.folder=getwd(), ensembl.url, threads=1){
	...
	if(group=="sudo"){
		...
		system(paste("sudo docker run -v ",genome.folder,":", genome.folder," -d docker.io/rcaloger/bwa.1 sh /bin/bwa.index.sh ",threads," ",ensembl.url, sep=""))
	}else{
		system("docker pull docker.io/rcaloger/bwa.1")
		system(paste("docker run -v ","ftp://ftp.ensembl.org/pub/release-87/fasta/homo_sapiens/dna/Homo_sapiens.GRCh38.dna.toplevel.fa.gz",":", genome.folder," -d docker.io/rcaloger/bwa.1 sh /bin/bwa.index.sh ",threads," ",ensembl.url, sep=""))
	}
}
